...
def calc_returns_asset_daily_absolute_analysisperiod(asset):
    ...
    cumu_inflow = list(itertools.accumulate(inflowlist))
    cumu_outflow = list(itertools.accumulate(outflowlist))

    ret = []
    for idx, _ in enumerate(datelist):
        cost = cumu_cost[idx]
        payout = cumu_payout[idx]
        inflow = cumu_inflow[idx]
        outflow = cumu_outflow[idx]
